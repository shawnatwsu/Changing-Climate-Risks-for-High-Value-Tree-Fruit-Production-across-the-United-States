...
from mpl_toolkits.basemap import Basemap  # Import Basemap for plotting maps

# Function to calculate trends for different frequencies (monthly, seasonal, annual)
def calculate_trends(da, time_dim, freq):
    if freq == 'monthly':
        resampled_da = da.resample({time_dim: '1MS'}).mean(dim=time_dim)  # Resample data to monthly means
    elif freq == 'seasonal':
        resampled_da = da.resample({time_dim: 'QS-DEC'}).mean(dim=time_dim)  # Resample data to seasonal means
    elif freq == 'annual':
        resampled_da = da.resample({time_dim: '1YS'}).mean(dim=time_dim)  # Resample data to annual means
    else:
        raise ValueError("Invalid frequency. Choose from 'monthly', 'seasonal', or 'annual'.")

    if freq == 'seasonal':
        resampled_da = resampled_da.assign_coords(season=resampled_da[time_dim].dt.season)  # Assign season coordinates
        trend_data = np.zeros((4, *da.shape[1:3]))  # Initialize array for seasonal trends
        ...
        trend_data = np.zeros((*da.shape[1:3]))  # Initialize array for monthly or annual trends
        for lat_idx in range(da.shape[1]):
            for lon_idx in range(da.shape[2]):
...
# Function to calculate monthly climatology
def calculate_monthly_climatology(da, time_dim):
    monthly_climatology = da.groupby(da[time_dim].dt.month).mean(dim=time_dim)  # Group by month and calculate mean
    return monthly_climatology

# Function to calculate monthly trends
def calculate_monthly_trends(da, time_dim):
    resampled_da = da.resample({time_dim: '1MS'}).mean(dim=time_dim)  # Resample data to monthly means
    trend_data = np.zeros((12, *da.shape[1:]))  # Initialize array for monthly trends

    for month in range(12):
        month_data = resampled_da.sel(day=resampled_da[time_dim].dt.month == (month + 1))  # Select data for each month
        for lat_idx in range(da.shape[1]):
            for lon_idx in range(da.shape[2]):
...
ds = xr.open_dataset('/home/shawn_preston/tmax/tmax_gridmet.nc')
...
trends = {}
resampled_data = {}
temp_data = ds_subset['tmax'] - 273.15  # Convert temperature from Kelvin to Celsius
...
monthly_climatology = calculate_monthly_climatology(ds_subset['tmax'] - 273.15, 'day')
...
for idx, month in enumerate(months):
    ...
    cbar.set_label('Maximum Temperature Trend (°C/decade)')
    plt.annotate('Data Source: Gridmet', xy=(0.01, 0.03), xycoords='axes fraction', fontsize=6, fontweight='bold', color='k')
    ax.set_title(f'{month} Maximum Temperature Trends (1979-2022)')

plt.savefig('/home/shawn_preston/FIGURES/Gridmet_monthly_tmax_trends_CONUS_1979_2022.png', dpi=300, bbox_inches='tight')
plt.show()
...
for idx, season in enumerate(seasons):
    ...
    cbar.set_label('Maximum Temperature Trend (°C/decade)')
    plt.annotate('Data Source: Gridmet', xy=(0.01, 0.03), xycoords='axes fraction', fontsize=6, fontweight='bold', color='k')
    ax.set_title(f'{season} Maximum Temperature Trends (1979-2022)')

plt.savefig('/home/shawn_preston/FIGURES/Gridmet_seasonal_tmax_trends_CONUS_1979_2022.png', dpi=300, bbox_inches='tight')
plt.show()
...
for idx, month in enumerate(months):
    ...
    plt.annotate('Data Source: Gridmet', xy=(0.01, 0.03), xycoords='axes fraction', fontsize=6, fontweight='bold', color='k')
    ax.set_title(f'{month} Max. Temp Climatology (1979-2022)')

plt.savefig('/home/shawn_preston/FIGURES/Gridmet_monthly_tmax_climatology_CONUS_1979_2022.png', dpi=300, bbox_inches='tight')
plt.show()
...
cbar = m.colorbar(im, location='bottom')
cbar.set_label('Maximum Temperature Trend (°C)')
plt.annotate('Data Source: Gridmet', xy=(0.01, 0.03), xycoords='axes fraction', fontsize=6, fontweight='bold', color='k')
ax.set_title(f'Annual Maximum Temperature Trends (1979-2022)')

plt.savefig('/home/shawn_preston/FIGURES/Gridmet_annual_tmax_trends_CONUS_1979_2022.png', dpi=300, bbox_inches='tight')
plt.show()
